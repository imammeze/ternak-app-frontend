...
const DAFTAR_AKTIVITAS = [
  ...
  "Pencatatan Data Sapi",
  "Perawatan Fasilitas / Alat",
];

export default function PresensiKeluarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post("/api/absen/keluar", payload);

      toast.success("Presensi Keluar Berhasil! Hati-hati di jalan.");
      router.push("/absen");
    } catch (err: any) {
  ...
  return (
    <>
      <div className="flex items-center px-4 pt-10 pb-4 bg-rose-600 text-white z-10 sticky top-0 shadow-sm">
      ...
      <div className="flex-1 bg-slate-50 px-5 pt-6 pb-32 overflow-y-auto">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 flex flex-col items-center justify-center mb-5 relative overflow-hidden">
          ...
          <h2 className="text-5xl font-black text-rose-600 tracking-tight z-10 tabular-nums">
            {isClient ? timeString : "--.--.--"}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center gap-2 mb-3">
              <ListTodo size={20} className="text-rose-500" />
              <h3 className="font-bold text-gray-800 text-sm">
                Aktivitas Hari Ini
              </h3>
            </div>
            <p className="text-xs text-gray-500 mb-4">
              Pilih pekerjaan yang telah Anda selesaikan hari ini (bisa lebih
              dari satu).
            </p>
              ...
              {DAFTAR_AKTIVITAS.map((item) => {
                ...
                return (
                  <label
                    ...
                    <span
                      className={`text-[14px] font-medium transition-colors ${isSelected ? "text-rose-700" : "text-gray-600"}`}>
                      {item}
                    </span>
                  </label>
                );
          ...
          <button
            ...
            className={`mt-2 w-full text-white font-bold py-4 rounded-xl shadow-lg transition-all tracking-wider flex justify-center items-center gap-2 ${
              isSubmitting || lokasiLoading || !!lokasiError
                ...
                : "bg-rose-500 hover:bg-rose-600 shadow-rose-200 active:scale-[0.98]"
            }`}>
            {isSubmitting ? (
