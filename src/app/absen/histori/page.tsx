  ...
  const rekapData = useMemo(() => {
    ...
    filteredHistori.forEach((item) => {
      ...
      if (item.tipe === "Masuk") {
        ...
        item.tipe === "Mulai Menginap" ||
        item.tipe === "Mulai Lembur"
      ) {
        result[userId].menginap += 1;
      }
    });
    ...
    <>
      ...
      <div className="flex-1 bg-slate-50 px-5 pt-6 pb-32 overflow-y-auto">
        {loading ? (
          ...
          <div className="text-center mt-10 text-rose-500 text-sm font-medium p-4 bg-rose-50 rounded-2xl">
            {errorMsg}
          </div>
        ) : histori.length === 0 ? (
              ...
              <>
                <div className="flex bg-gray-200/50 p-1 rounded-xl mb-2">
                  <button
                    ...
                    className={`flex-1 flex justify-center gap-2 py-2.5 text-sm font-bold rounded-lg ${activeTab === "riwayat" ? "bg-white text-emerald-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                    <List size={16} /> Riwayat
                  </button>
                  <button
                    onClick={() => setActiveTab("rekapan")}
                    className={`flex-1 flex justify-center gap-2 py-2.5 text-sm font-bold rounded-lg ${activeTab === "rekapan" ? "bg-white text-emerald-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
